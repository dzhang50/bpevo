...
#define G_SIZE 18 // 256K 2-bit counters = 64 KB cost
...
uint32_t mix(uint32_t b, uint32_t a)
{
...
void PredictorReset() {
  ...
  printf("Predictor:ghash\nconfig: %i counters, %i KB cost\n", 1 << G_SIZE, (1 << G_SIZE) * 2 / 8 / 1024);
 
  for (int i = 0; i < (1 << G_SIZE); i ++)
    gtable[i] = 0;
...
void PredictorRunACycle() {
  ...
  for (int i = 0; i < cycle_info->num_fetch; i++) {
    ...
    if (uop->type & IS_BR_CONDITIONAL) {
      // get prediction
      uint32_t gidx = mix(brh_fetch,uop->pc) & ((1 << G_SIZE) - 1);
  ...
  for (int i = 0; i < cycle_info->num_retire; i++) {
    ...
    if (uop->type & IS_BR_CONDITIONAL) {
      uint32_t gidx = mix(brh_retire, uop->pc) & ((1 << G_SIZE) - 1);
