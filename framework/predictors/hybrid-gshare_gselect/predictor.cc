...
void PredictorRunACycle() {
  ...
  for (int i = 0; i < cycle_info->num_retire; i++) {
	...
	bool gshare_prediction =  (gshare_table[gshare_idx] >= 0);
	bool gselect_prediction = (gselect_table[gselect_idx] >= 0);

	bool t = uop->br_taken;	
	// update meta predictor
	UpdateMetaTable((t == gshare_prediction)?1:0, (t == gselect_prediction)?1:0, meta_table, meta_idx);
